...
st.set_page_config(page_title="Dashboard Superstore", page_icon=":bar_chart:")
st.markdown("""
    <style>
    body {
...
with tab2:
    ...
    - O grau de dispersão dos pontos ao redor da linha mostra o quanto os dados seguem essa tendência. 
        ...
        - Outra questão, quando as vendas são baixas, os **pontos estão concentrados** próximos à linha de regressão, indicando um **padrão estável de lucro**. 
    ...
    sns.scatterplot(data=df, x='Vendas', y='Lucro', hue='Categoria', 
                    palette="mako", as_cmap=True, alpha=0.7, s=200, edgecolor='black', linewidth=1.5)
    ...
    barplot = sns.barplot(data=df, x='Categoria', y='Margem de Lucro', 
                          palette="mako", as_cmap=True, alpha=0.7, estimator=np.mean)
