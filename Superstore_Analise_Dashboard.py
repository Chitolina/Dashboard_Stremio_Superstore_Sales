...
with tab6:
    ...
    sns.barplot(data=top_states, x='Profit', y='State', hue='Region', palette=region_colors, ax=ax1)
    for p in ax1.patches:
        x = p.get_width()
        y = p.get_y() + p.get_height() / 2
        ax1.text(x, y, f'{x:,.0f}', ha='left', va='center', fontsize=12)
    ...
    sns.barplot(data=sales_by_region, x='Region', y='Sales', palette=region_colors, ax=ax2)
    for p in ax2.patches:
        ax2.text(p.get_x() + p.get_width() / 2, p.get_height(), f'{p.get_height():,.0f}', ha='center')
    ...
    sns.scatterplot(data=cluster_data, x='Sales', y='Profit', hue='Cluster', palette="Set1", s=150, alpha=0.7, ax=ax3)
    ax3.set_title("Clusterização dos Estados por Desempenho", fontsize=18, fontweight='bold')
    st.pyplot(fig3)
    
    # Gráfico 4: Heatmap de Correlações por Região
    ...
    for i, region in enumerate(regions):
        ...
        axes[i].set_title(f'Correlações - {region}', fontsize=14)
    st.pyplot(fig)
